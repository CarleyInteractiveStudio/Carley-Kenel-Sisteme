...
context_t *irq_handler(uint64_t int_no, context_t *ctx) {
    ...
    if (int_no < 32) {
        video_clear(0x880000); // Fondo rojo de error
        if (int_no == 14) {
            uint64_t cr2;
            __asm__ volatile("mov %%cr2, %0" : "=r"(cr2));
            video_draw_string("PAGE FAULT", 10, 10, 0xFFFFFF);
            /* En un microkernel real, intentaríamos corregir o matar el proceso */
        } else if (int_no == 13) {
            video_draw_string("GENERAL PROTECTION FAULT", 10, 10, 0xFFFFFF);
        } else {
            video_draw_string("CPU EXCEPTION", 10, 10, 0xFFFFFF);
        }
        for (;;) { __asm__("hlt"); }
    }
